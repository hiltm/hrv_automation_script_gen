...
def write_text(self,passed_string):
	f = self.controller.shared_data["configFile"]
	open(f, "w")
...
class repetitionSummary(tk.Frame):
	def __init__(self, parent, controller):
		...
		buttonConfirm = tk.Button(self, text="Yes", command = lambda : [write_text(self,"Vb:1"),increment_counter(),controller.show_frame(parameterTabs)])
